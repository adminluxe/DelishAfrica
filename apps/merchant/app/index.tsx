  ...
  const moment = useMemo<ServiceMoment>(() => {
    ...
    if (presenceNetworkLocked) {
      return {
        ...
        title: "Revalidez le réseau avant de changer le service.",
        body: lastSync === "—"
          ? "Aucune présence ne sera modifiée tant que la vérité réseau n’est pas confirmée."
          : `Dernière confirmation ${lastSync}. Les commandes déjà connues restent accessibles.`,
        action: "Réessayer",
        activeStage: 0,
      };
    }
    ...
    return baseMoment;
  }, [activationProofCount, activationProofs.length, activationReady, baseMoment, lastSync, networkTruth, presenceHydrated, presenceMutation, presenceMutationBusy, presenceNetworkLocked, priority, serviceOpen]);
  ...
  async function togglePresence() {
    ...
    if (presenceNetworkLocked) {
      Alert.alert(
        ...
        "Le dernier état reste protégé. Revalidez la connexion avant de changer la présence du service.",
        [
    ...
    <MerchantAquaticSignature reduceMotion={reduceMotion}>
      ...
      >
        <View style={styles.topbar}>
          ...
          >
            ...
            <View>
              ...
              <Text style={styles.presenceHint}>{!presenceHydrated ? "Reprise sécurisée" : !activationReady ? "Compléter l’espace" : presenceMutationBusy ? "Écriture puis relecture" : networkTruth === "syncing" ? "Vérité en cours" : presenceNetworkLocked ? `Confirmé ${lastSync}` : `Synchro ${lastSync}`}</Text>
            </View>
          </Pressable>
        </View>
        ...
        {activationPassportVisible ? null : (
          <>
            <Text style={styles.manifesto}>La prochaine action, sans détour.</Text>
            <Text style={styles.manifestoBody}>
              Accepter, préparer ou remettre : le cockpit place le geste utile avant tout le reste.
            </Text>
          </>
        )}
        ...
        >
          ...
          {networkTruth === "stale" && error && orders.length > 0 ? (
            <Pressable
              ...
              onPress={() => { void load(); }}
              accessibilityRole="button"
              accessibilityLabel="Dernière vérité conservée. Réessayer la synchronisation du service"
            >
              <View style={{ flex: 1 }}>
          ...
          {activationPassportVisible ? null : loading ? (
            ...
            <Pressable
              ...
              onPress={() => { void load(); }}
              accessibilityRole="button"
              accessibilityLabel="Connexion à vérifier. Réessayer la synchronisation du service"
            >
              <Text style={styles.errorTitle}>Connexion à vérifier</Text>
              <Text style={styles.errorText}>{error}</Text>
              <Text style={styles.errorRetry}>Touchez pour réessayer</Text>
            </Pressable>
          ) : priority ? (
