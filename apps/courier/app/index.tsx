...
export default function CourierHome() {
  ...
  const moment = useMemo<TerrainMoment>(() => {
    ...
    if (presenceNetworkLocked) {
      return {
        mode: "VEILLE",
        kicker: "LA DERNIÈRE VÉRITÉ RESTE VISIBLE",
        title: "Revalidez le réseau avant de changer le terrain.",
        body: lastSync === "—"
          ? "Aucune disponibilité ne sera modifiée tant que la vérité réseau n’est pas confirmée."
          : `Dernière confirmation ${lastSync}. Les missions déjà connues restent accessibles.`,
        action: "Réessayer",
        stage: 0,
      };
    }
    ...
    return baseMoment;
  }, [activationProofCount, activationProofs.length, activationReady, available, baseMoment, lastSync, networkTruth, presenceHydrated, presenceMutation, presenceMutationBusy, presenceNetworkLocked, priority]);

  async function togglePresence() {
    if (!presenceHydrated || presenceMutationRef.current !== "idle") return;
    if (presenceNetworkLocked) {
      Alert.alert(
        "Vérité réseau à confirmer",
        "Le dernier état reste protégé. Revalidez la connexion avant de changer votre présence terrain.",
        [
          { text: "Plus tard", style: "cancel" },
          { text: "Réessayer", onPress: () => { void refreshLiveTruth(); } },
        ],
      );
      return;
    }
    ...
    <CourierAquaticSignature reduceMotion={reduceMotion}>
      ...
      >
        <View style={styles.topbar}>
          ...
          >
            ...
            <View>
              ...
              <Text style={styles.presenceHint}>{!presenceHydrated ? "Reprise sécurisée" : !activationReady ? "Compléter le terrain" : presenceMutationBusy ? "Écriture puis relecture" : networkTruth === "syncing" ? "Vérité en cours" : presenceNetworkLocked ? `Confirmé ${lastSync}` : "Touchez pour changer"}</Text>
            </View>
          </Pressable>
        </View>
        ...
        {activationPassportVisible ? null : (
          <>
            ...
            <Text style={styles.heroBody}>
              La prochaine adresse et l’unique action utile restent visibles avant tout le reste.
            </Text>
          </>
        )}
        ...
        >
          ...
          {networkTruth === "stale" && error && orders.length > 0 ? (
            <Pressable
              ...
              onPress={() => { void load(); }}
              accessibilityRole="button"
              accessibilityLabel="Dernière vérité conservée. Réessayer la synchronisation du terrain"
            >
              <View style={{ flex: 1 }}>
                <Text style={styles.networkNoticeKicker}>DERNIÈRE VÉRITÉ CONSERVÉE</Text>
                <Text style={styles.networkNoticeText}>{error}</Text>
                {lastSync !== "—" ? <Text style={styles.networkNoticeMeta}>Dernière confirmation {lastSync}</Text> : null}
              </View>
          ...
          {activationPassportVisible ? null : loading ? (
            ...
            <Pressable
              ...
              onPress={() => { void load(); }}
              accessibilityRole="button"
              accessibilityLabel="Connexion à vérifier. Réessayer la synchronisation du terrain"
            >
              <Text style={styles.inlineErrorTitle}>Connexion à vérifier</Text>
              <Text style={styles.inlineErrorText}>{error}</Text>
              <Text style={styles.inlineErrorRetry}>Touchez pour réessayer</Text>
            </Pressable>
          ) : priority ? (
        ...
        <Pressable
          ...
          accessibilityRole="button"
          accessibilityLabel="Ouvrir le suivi terrain Courier"
        >
