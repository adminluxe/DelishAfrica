...
const INSTALLATION_KEY = 'da_courier_installation_id_v1';
...
const LEGACY_MIGRATION_OWNER_KEY = 'da_courier_legacy_migration_owner_v2';
...
function sessionSubject(session: any, fallback = ''): string {
  ...
  return String(user.id || user.courierId || fallback || '').trim();
}
...
async function bindActiveScope(scope: string): Promise<void> {
  ...
  if (previous && previous !== scope) {
    delete (globalThis as any).__DELISHAFRICA_COURIER_PROFILE_LITE_V1__;
  }
