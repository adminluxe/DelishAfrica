...
export function daAuthApiBaseUrl(): string {
return apiBase();
}

export function daSessionLabel(session: DaAuthSession | null, role?: DaRole): string {
const active = Boolean(session?.authenticated);
const resolvedRole = session?.user?.role || role || "courier";

if (active) {
if (resolvedRole === "courier") return "Session coursier active";
if (resolvedRole === "merchant") return "Session restaurateur active";
if (resolvedRole === "client") return "Session client active";
return "Session active";
}

return "Session progressive prête";
}
