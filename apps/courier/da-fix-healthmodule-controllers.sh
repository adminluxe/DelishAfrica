# === da-fix-healthmodule-controllers.sh ===
set -euo pipefail
...
# 1) Ajouter l'import HealthModule si manquant
node - <<'NODE'
const fs=require('fs'); const p=process.env.APP;
let s=fs.readFileSync(p,'utf8');

// Supprime un éventuel import HealthController direct
s = s.replace(/^\s*import\s*{\s*HealthController\s*}\s*from\s*'\.\/health\/health\.controller';?\s*$/m, '');

if (!/from\s*'\.\/health\/health\.module'/.test(s)) {
  // Insère l'import tout en haut, après le premier import
  s = s.replace(/(^import[^\n]*\n)/, `$1import { HealthModule } from './health/health.module';\n`);
}

// Garantir HealthModule dans 'imports: [ ... ]'
...
// Retirer HealthModule et HealthController de 'controllers: [ ... ]' s'ils s'y trouvent
...
console.log('✔ Patch appliqué à', p);
NODE
APP="$APP" node -e "$(tail -n +1 <<'NODE'
const fs=require('fs'); // no-op, placeholder (already executed patch above)
NODE
)"

# 2) Rebuild + (re)start PM2
cd "$API"
