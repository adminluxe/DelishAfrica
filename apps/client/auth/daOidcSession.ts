...
function daC2SessionStage(stage: string, detail?: Record<string, unknown>): void {
  // SAFE TRACE: stage names / booleans / error codes only. Never tokens, claims, nonce or userinfo.
  console.log(`[DA_C2_SESSION_STAGE] ${JSON.stringify({ stage, ...(detail || {}) })}`);
}
...
export async function establishOidcSession(
  ...
  daC2SessionStage('ESTABLISH_ENTER');
  try {
    const tokenSet = normalizeTokenSet(tokenResponse);
    daC2SessionStage('NORMALIZE_TOKEN_SET_OK');
    const validated = validateInitialTokenSet(
      tokenSet.accessToken,
      tokenSet.idToken,
      expectedNonce,
    );
    daC2SessionStage('VALIDATE_INITIAL_TOKEN_SET_OK');
    const doc = await discovery();
    daC2SessionStage('DISCOVERY_OK');
    const userInfo = await fetchUserInfo(tokenSet.accessToken, doc);
    daC2SessionStage('USERINFO_OK');
    assertUserInfoMatches(validated.access.claims, userInfo);
    daC2SessionStage('USERINFO_MATCH_OK');
    const safe = toSafeSession(validated.access.claims, userInfo);
    daC2SessionStage('SAFE_SESSION_OK', { authenticated: safe.status === 'authenticated' });
    await writeOidcVault(tokenSet, {
      subject: safe.subject,
      displayName: safe.displayName,
      email: safe.email,
    });
    daC2SessionStage('VAULT_WRITE_OK');
    return safe;
  } catch (error) {
    daC2SessionStage('ESTABLISH_FAILED', {
      errorName: error instanceof Error ? error.name : 'unknown',
      errorCode: error instanceof DaOidcError ? error.code : 'unknown',
      recoverable: error instanceof DaOidcError ? error.recoverable : false,
    });
    throw error;
  }
}
