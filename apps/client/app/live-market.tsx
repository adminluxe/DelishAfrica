...
import { WaterRadarV2, type WaterRadarSignal } from "../ui/water/WaterRadarV2";
import {
...
function networkTruthMessage(error: unknown) {
  ...
  if (/temps|timeout|network|fetch|connexion|http/i.test(detail)) {
    return "Connexion momentanément indisponible. Vos dernières données restent disponibles.";
  }
  return "Une partie du réseau ne répond pas encore. Vos dernières données restent disponibles.";
}
...
export default function GlobalMarketplaceHome() {
  ...
  const radarLeadPartner = ambientSignal || orderNowPartner;
  const waterRadarSignal = useMemo<WaterRadarSignal | undefined>(() => {
    if (!radarLeadPartner) return undefined;
    return {
      name: String(radarLeadPartner.name || "Signal DelishAfrica"),
      meta: `${cityOf(radarLeadPartner)} · ${countryOf(radarLeadPartner)} · ${cuisineOf(radarLeadPartner)}`,
      kind: radarLeadPartner.marketplaceSource,
    };
  }, [radarLeadPartner]);
    ...
    >
      ...
      <View style={styles.header}>
        <View>
          ...
          <Text style={styles.role}>Marketplace vivante · Commande · Suivi</Text>
        </View>
        <Pressable
      ...
      <View style={styles.futureRail}>
        ...
        <View style={{ flex: 1 }}>
          <Text style={styles.futureRailKicker}>DELISHAFRICA LIVE MARKET</Text>
          <Text style={styles.futureRailText}>
            {networkTruth === "live"
              ? "Tables ouvertes synchronisées"
              : networkTruth === "stale"
                ? "Dernières données actives · relance en arrière-plan"
                : "Synchronisation discrète du marché"}
          </Text>
        </View>
      ...
      <View style={styles.hero}>
        <Text style={styles.eyebrow}>AFRICA · LIVE · À PORTÉE DE GESTE</Text>
        <Text style={styles.title}>Choisissez une table. Le reste se met en mouvement.</Text>
        <Text style={styles.subtitle}>Restaurants, plats, commande et suivi composent un seul parcours. Explorez maintenant, gardez le contexte jusqu’à votre porte.</Text>
        <View style={styles.searchBox}>
          <Text style={styles.searchIcon}>⌕</Text>
          <TextInput
            value={query}
            onChangeText={setQuery}
            placeholder="Une envie, une table, une ville…"
      ...
      <WaterRadarV2
        ...
        hasActiveOrder={Boolean(activeOrders[0])}
        activeStage={activeStage}
        signal={waterRadarSignal}
        reduceMotion={reduceMotion}
        onOpenPulse={() => {
          rememberMarketplaceFocus();
          router.push("/market-pulse" as never);
        }}
      ...
      <View style={styles.essentialCard}>
        ...
        <View style={styles.essentialFacts}>
          ...
          <View style={styles.essentialFact}>
            ...
            <Text style={styles.essentialFactLabel}>pays explorés</Text>
          </View>
          <View style={styles.essentialFact}>
            <Text style={styles.essentialFactValue}>{activeOrders.length ? activeStage + 1 : radarCount}</Text>
            <Text style={styles.essentialFactLabel}>{activeOrders.length ? "étape en cours" : "adresses repérées"}</Text>
          </View>
        </View>
        <Pressable
      ...
      {networkTruth === "stale" && error ? (
        <Pressable
          ...
          onPress={() => { void load(); }}
          accessibilityRole="button"
          accessibilityLabel="Mode dégradé. Relancer la synchronisation"
        >
          <View style={{ flex: 1 }}>
            ...
            {lastConfirmedAt !== "—" ? (
              <Text style={styles.networkNoticeMeta}>Dernière synchro {lastConfirmedAt}</Text>
            ) : null}
          </View>
      ...
      <Text style={styles.networkWhisper}>
        {liveCount} partenaires actifs · {radarCount} adresses repérées · {countryCount} pays explorés · marché sous contrôle.
      </Text>
      </ScrollView>
    </AquaticSignature>
  );
...
const styles = StyleSheet.create({
  ...
  futureRailPulse: {
    ...
    height: 10,
    borderRadius: 99,
    backgroundColor: "#D9AE68",
    shadowColor: "#D9AE68",
    shadowOpacity: 0.38,
    shadowRadius: 10,
  },
  futureRailPulseLive: { backgroundColor: "#8FE2C0", shadowColor: "#8FE2C0" },
  futureRailKicker: { color: "#8FE2C0", fontSize: 8, fontWeight: "900", letterSpacing: 1.8 },
  futureRailText: { color: "rgba(255,248,234,0.76)", fontSize: 10.5, lineHeight: 15, fontWeight: "700", marginTop: 3 },
  futureRailMeta: { color: "#D9AE68", fontSize: 9, fontWeight: "900", letterSpacing: 0.9, maxWidth: 76, textAlign: "right" },
