...
function hexToRgb(hex: string) {
  ...
  if (h.length !== 6) return { r: 255, g: 106, b: 42 };
...
export function makeTheme(accent: DelishAccent = "merchant") {
  const a = (accent || "merchant").toString().trim().toLowerCase();
