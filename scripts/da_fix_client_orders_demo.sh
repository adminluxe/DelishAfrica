...
APP_DIR="$ROOT/apps/client"
APP_APP_DIR="$APP_DIR/app"
TS="$(date +%Y%m%d-%H%M%S)"
BK="$ROOT/tonton_backups/client_orders_demo_$TS"
LOG="$ROOT/tonton_logs/client_orders_demo_fix_$TS.log"

mkdir -p "$BK" "$(dirname "$LOG")"

log(){ echo "[$(date +%H:%M:%S)] $*" | tee -a "$LOG" ; }

if [[ ! -d "$APP_APP_DIR" ]]; then
  log "❌ Dossier introuvable: $APP_APP_DIR"
  exit 1
fi

log "== Client orders-demo FIX =="
log "ROOT=$ROOT"
log "APP_APP_DIR=$APP_APP_DIR"
log "LOG=$LOG"
log "BK=$BK"

# 1) Find orders-demo route file (most common names)
CANDIDATES=()
while IFS= read -r -d '' f; do CANDIDATES+=("$f"); done < <(find "$APP_APP_DIR" -maxdepth 5 -type f \( -name "orders-demo.tsx" -o -name "orders-demo.jsx" -o -name "orders-demo.ts" -o -name "orders-demo.js" \) -print0 2>/dev/null || true)

# fallback: find file mentioning "Créer une commande" or "Thieyp" & "orderId"
if [[ ${#CANDIDATES[@]} -eq 0 ]]; then
  log "ℹ️ orders-demo.* non trouvé. Recherche par contenu (Créer une commande / Thieyp / orderId)..."
  while IFS= read -r -d '' f; do CANDIDATES+=("$f"); done < <(
    grep -RIl --null -E "Créer une commande|Thieyp|orderId|demo/create|orders/demo" "$APP_APP_DIR" 2>/dev/null | head -z -n 10 || true
  )
fi

if [[ ${#CANDIDATES[@]} -eq 0 ]]; then
  log "❌ Aucun fichier écran Client trouvé à patcher."
  log "➡️ Donne-moi le path exact du screen où tu cliques 'Créer une commande Thieyp'."
  exit 1
fi

TARGET="${CANDIDATES[0]}"
log "✅ Fichier cible: $TARGET"
log "ℹ️ Autres candidats (si besoin):"
for f in "${CANDIDATES[@]}"; do log " - $f"; done

# backup
cp -a "$TARGET" "$BK/$(basename "$TARGET").bak"
log "Backup: $BK/$(basename "$TARGET").bak"

# 2) Write a robust screen (TypeScript React Native / Expo Router friendly)
cat > "$TARGET" <<'TSX'
...
log "✅ Screen réécrit: $TARGET"
log "➡️ Relance Client avec cache clear (obligatoire)."
log "DONE."
