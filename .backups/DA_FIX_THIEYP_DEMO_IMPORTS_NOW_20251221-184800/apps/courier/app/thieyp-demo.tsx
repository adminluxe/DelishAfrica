...
import { getApiBase, demoReset, demoCreate, demoList, demoGet, demoSetStatus, DemoOrderItem } from "./orders-";
...
export default function ThieypDemoScreen() {
 ...
 <Text style={{ fontSize: 22, fontWeight: "800" }}>Thieyp — {role}</Text>
 <Text style={{ opacity: 0.7 }}>API: {base}</Text>

 <View style={{ flexDirection: "row", flexWrap: "wrap" }}>
 <Btn title="Reset " disabled={busy} onPress={() => run(() => demoReset(base))} />
 {role === "client" ? (
 <Btn
 ...
 Astuce: ouvre la route /thieyp- dans Expo Router (URL finissant par --/thieyp-).
 </Text>
